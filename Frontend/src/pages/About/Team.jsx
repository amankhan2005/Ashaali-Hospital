...
const Team = () => {
  ...
  // ✅ safe defaults
  const mainDoctor = doctors?.[0] || null;
  const otherDoctors = doctors?.slice(1) || [];

  // ✅ inside component
  const handleBookClick = (doctor) => {
    navigate(
      `/book-appointment?doctorId=${doctor._id}&department=${doctor.department}&doctorName=${encodeURIComponent(
        doctor.name
      )}`
    );
  };
                  ...
                  <div className="lg:w-3/5 p-8 md:p-12">
                    <div className="flex flex-col h-full">
                      ...
                      <div className="">
                        <button
                          onClick={() => handleBookClick(mainDoctor)}
                          className="px-14  py-4 rounded-xl font-bold text-lg shadow-lg transition-all duration-300 transform hover:scale-105 bg-[#18978d] text-white hover:bg-[#147a71] hover:shadow-xl"
                        >
                          Book Appointment 
                        </button>
                      </div>
