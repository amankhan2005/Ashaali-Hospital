...
const Footer = () => {
  const facilities = ['ICU', 'Emergency', 'Ventilator', 'Ambulance', 'XRay', 'Pathology', 'Physiotherapy'];
  const specialties = ['Orthopaedics', 'Ophthalmology', 'Pediatrics', 'Neurology', 'General Medicine', 'ENT', 'Gastrology', 'General Surgery', 'Obstetrics & Gynaecology', 'Urology', 'Nephrology', 'Dental', 'Hematology'];
 ...
 <div className="space-y-3 text-gray-300 text-sm">
  ...
  <div className="flex items-start gap-3">
    ...
    >
      E2/PH-7, Amrapali Yojana, IIM ROAD, near Joggers Park Chauraha,
          ...
          <div className="flex flex-col">
            ...
            <ul className="mt-3 space-y-2 text-gray-300 text-sm">
              {[
                ...
                <li key={idx}>
                  <Link to={item.path} className="flex items-center gap-2 hover:text-white transition font-medium py-1">
 ...
 <div className="flex flex-col sm:col-span-2 lg:col-span-1 lg:pr-8 xl:pr-12">
  ...
  {/* Equal row alignment grid */}
  <div className="mt-3 grid grid-cols-2 gap-x-16 text-gray-300 text-sm">
    {/* Left + Right lists aligned by row */}
    {leftSpecialties.map((leftItem, idx) => (
      <React.Fragment key={idx}>
        ...
        <div className="flex items-center gap-2 py-1">
          <FiChevronRight className="w-4 h-4 flex-shrink-0 text-gray-400" />
          <Link
            to={`/department/${leftItem.toLowerCase().replace(/\s+/g, '-')}`}
        ...
        {rightSpecialties[idx] ? (
          ...
          <div /> // blank space if right side has fewer items
        )}
      </React.Fragment>
    ))}
  </div>
          ...
          <div className="flex flex-col ml-10 sm:col-span-2 lg:col-span-1 lg:pl-8 xl:pl-12">
            ...
            <ul className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-y-2 text-gray-300 text-sm">
              {facilities.map((item, idx) => (
                <li key={idx}>
                  <Link to={`/facility/${item.toLowerCase().replace(/\s+/g, '-')}`} className="flex items-center gap-2 hover:text-white transition py-1">
                    <FiChevronRight className="w-4 h-4 flex-shrink-0 text-gray-400" />
